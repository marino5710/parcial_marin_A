...
const consultarMorty = async (e) => {
    ...
    const consultarEpisodio = async (e) => {
        e.preventDefault();
        let nombreEpisodio = formulario1.episodio.value;
        if (nombreEpisodio == '') {
          alert("Debe ingresar el nombre del episodio");
          return;
        }
        const url = `https://rickandmortyapi.com/api/character/?name=${nombreEpisodio}&status=alive`;
        const config = {
          method: 'GET'
        };
      
        document.getElementById('estadoEpisodio').innerText = 'Buscando personaje...';
        try {
            const respuesta = await fetch(url, config);
            if (respuesta.status === 200) {
              const data = await respuesta.json();
